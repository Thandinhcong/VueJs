  ...
  <form class="flex h-12 w-full items-center rounded-3xl border border-solid border-brand-gray-3"
    ...
    <div class="flex flex-1 flex-nowrap h-full text-base font-light">
      ...
      <div class="relative flex h-full flex-1 items-center pl-3">
        <label for="localtion" class="absolute left-0 -top-10">Where?</label>
        <text-input id="localtion" placeholder="Los Angeles" v-model="localtion" />
      </div>
...
export default {
  ...
  methods: {
    searchForJobs() {
      this.$router.push({
        name: "JobResults",
        query: { role: this.role, localtion: this.localtion }
      })
    }
  }
};
